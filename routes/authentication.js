...
router.post(
  ...
  asyncHandler(async function (req, res) {
    ...
    console.log(`http://127.0.0.1:${process.env.PORT}/templates/activate.html?token=${token}`);

    sendEmail(
      [email],
      `[${process.env.MAIL_APP}] User activation`,
      `For activation of your user account, you need to access the following link http://127.0.0.1:${process.env.PORT}/templates/activate.html?token=${token}`
    );
